"""fal entrypoint only; app in colmap_impl."""
import textwrap
...
def colmap():
    ...
    print("[fal-native/colmap] entry", flush=True)
    _ensure_syspath(inspect.currentframe().f_globals, "colmap")

    svc = importlib.import_module(_IMPL)
    print("[fal-native/colmap] uvicorn on 0.0.0.0:8000", flush=True)
